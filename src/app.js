...
(function () {
  ...
  const postToGameFrame = (frame, type, payload = {}) => {
    ...
    try {
      frame.contentWindow.postMessage({ kind: 'vitrina:game-host', bridgeId: state.bridgeId, type, payload }, '*');
      return true;
    } catch {
      return false;
    }
  };
  const getGameCapability = gameId => String(state.capabilities.games?.[String(gameId || '').trim()] || '');
  ...
  const applySnapshot = snapshot => {
    ...
    if (shards) {
      ...
      shards.title = wallet.available ? 'Баланс Осколков' : 'Экономика готовится к запуску';
    }
  ...
  const openFriendsTab = async () => {
    ...
    document.querySelectorAll('.bt-nav-item').forEach(button => button.classList.remove('is-active'));
    $('nav-friends')?.classList.add('is-active');
    const title = document.querySelector('.bt-title h1');
    if (title) title.textContent = 'Друзья';
    document.querySelectorAll('.bt-hotspot').forEach(node => {
    ...
    `;
    try {
      const moduleUrl = new URL('./common/friends-embed.js?v=9.2.7', document.baseURI).href;
  ...
  const openGame = gameId => {
    ...
    if (existingFrame) {
      ...
      fitWorld();
      postToGameFrame(existingFrame, 'GC_INIT', { bridgeId: state.bridgeId, gameId: game.id, capabilityToken: getGameCapability(game.id), snapshot: state.snapshot, at: Date.now() });
    ...
    if (params.get('join')) {
      launchUrl.searchParams.set('join', params.get('join'));
    }
    if (params.get('inviteFriend')) {
      launchUrl.searchParams.set('inviteFriend', params.get('inviteFriend'));
    }
    ...
    const postToGame = () => {
      postToGameFrame(frame, 'GC_INIT', { bridgeId: state.bridgeId, gameId: game.id, capabilityToken: getGameCapability(game.id), snapshot: state.snapshot, at: Date.now() });
  ...
  const restoreActiveGame = preferredGameId => {
    ...
    if (gameHost) {
      const frame = activeId ? gameHost.querySelector(`.bt-game-frame[data-game-id="${activeId}"]`) : gameHost.querySelector('.bt-game-frame');
      if (frame) {
        const panel = $('bt-panel');
        if (panel) {
          panel.hidden = true;
        ...
        postToGameFrame(frame, 'GC_INIT', { bridgeId: state.bridgeId, gameId: activeId, capabilityToken: getGameCapability(activeId), snapshot: state.snapshot, at: Date.now() });
  ...
  const openFame = async () => {
    ...
    try {
      ...
      list.innerHTML = res.leaders
        .map(
          (p, i) => `
        <div class="bt-game-card" style="min-height: 70px; grid-template-columns: 42px 1fr auto; padding: 10px;">
          <div class="bt-game-icon" style="width:42px; height:42px; font-size:18px; border-radius:12px;">${p.avatarUrl ? `<img src="${String(p.avatarUrl).replace(/"/g, '&quot;')}" style="width:100%;height:100%;border-radius:12px;object-fit:cover">` : i === 0 ? '👑' : i === 1 ? '🥈' : i === 2 ? '🥉' : '👤'}</div>
  ...
  const bindBridge = () => {
    window.addEventListener('message', e => {
      ...
      if (d.type === 'GC_INIT') {
        ...
        const capabilities = d.payload?.capabilities && typeof d.payload.capabilities === 'object' ? d.payload.capabilities : {};
        state.capabilities = { tower: String(capabilities.tower || ''), games: capabilities.games && typeof capabilities.games === 'object' ? { ...capabilities.games } : {} };
        window.__GC_CAPABILITY_TOKEN = state.capabilities.tower;
      ...
      if (d.type === 'GC_SIGNALING_RESPONSE') {
        const gameIframe = document.querySelector('.bt-game-frame');
        if (gameIframe) {
          postToGameFrame(gameIframe, 'GC_SIGNALING_RESPONSE', d.payload || {});
        }
        return;
      }
      if (d.type === 'GC_FRIENDS_RESPONSE') {
        const gameIframe = document.querySelector('.bt-game-frame');
        if (gameIframe) {
          postToGameFrame(gameIframe, 'GC_FRIENDS_RESPONSE', d.payload || {});
        }
        return;
      }
    ...
    if (isStandalone()) {
      ...
      applySnapshot({ user: { displayName: 'Standalone' }, progress: { level: 1, xp: 0, achievementsUnlocked: 0, achievementsTotal: 0 }, wallet: { available: false, shards: 0, locked: 0, version: 0 }, player: { title: '' } });
    }
  };
  const bindScrollProxy = () => {
    ...
    scene.addEventListener(
      'wheel',
      event => {
        send('GC_PARENT_SCROLL', { deltaY: event.deltaY, at: Date.now() });
      },
      { passive: true }
    );
  };
  ...
  const init = () => {
    ...
    window.addEventListener('message', e => {
      ...
      const gameIframe = document.querySelector('.bt-game-frame');
      if (gameIframe?.contentWindow && e.source !== gameIframe.contentWindow) return;
      const gameId = gameIframe?.dataset?.gameId || state.activeGameId || '';
      if (d.type === 'GC_SIGNALING_REQUEST') {
        send('GC_SIGNALING_REQUEST', { ...(d.payload || {}), capabilityToken: getGameCapability(gameId) });
        return;
      }
      if (d.type === 'GC_FRIENDS_REQUEST') {
        send('GC_FRIENDS_REQUEST', { ...(d.payload || {}), capabilityToken: getGameCapability(gameId) });
        return;
      }
      if (d.type === 'GC_CLOSE') {
        closeGameHost();
      ...
      if (d.type === 'GC_COLLAPSE_GAME') {
        ...
        send('GC_COLLAPSE_GAME', { ...(d.payload || {}), gameId: state.activeGameId, at: Date.now() });
        return;
      }
      if (d.type === 'GC_SAVE_DATA') {
        send('GC_SAVE_DATA', { ...(d.payload || {}), capabilityToken: getGameCapability(gameId) });
        return;
      }
      ...
      if (d.type === 'GC_READY' || d.type === 'GC_REQUEST_SNAPSHOT') {
        ...
        if (gameIframe) {
          if (d.type === 'GC_READY') {
            postToGameFrame(gameIframe, 'GC_INIT', { bridgeId: state.bridgeId, gameId: state.activeGameId, capabilityToken: getGameCapability(state.activeGameId), snapshot: state.snapshot, at: Date.now() });
    ...
    if (launchParams.get('gcGame') === 'war_hearts' || launchParams.get('game') === 'war_hearts') {
      if (launchParams.get('join') || launchParams.get('inviteFriend')) {
